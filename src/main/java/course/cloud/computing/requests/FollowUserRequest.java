...
public class FollowUserRequest implements WorkItem {
	
	boolean isProcessed = false;
	int userId = 0;
	int friendId = 0;
	User response = null;
	public FollowUserRequest(int id,int fid)
	{
		userId = id;
		friendId = fid;
	}
	...
	public boolean process() {
		response = new User();
		try {
			response.setId(SocialNetworkDataBase.followUser(userId, friendId));
